...
function calculate(map, hits, type = "pulse") {
	let diff = maps[map].difficulty;
	let notes = (typeof maps[map].notes == "number" && !isNaN(maps[map].notes)) ? maps[map].notes : hits[0]+hits[1]+hits[2]+hits[3]+hits[4]
	if (maps[map].notes == undefined) console.warn(`This kid, ${maps[map].name} has an unknown amount of notes, so it will be treated as the amount of hits (${notes})`);
